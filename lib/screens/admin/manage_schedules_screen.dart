...
class _ManageSchedulesScreenState extends State<ManageSchedulesScreen> {
  ...
  Future<void> _submit() async {
    ...
    try {
      await SheetsApiService.post('addJadwal', {
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text('Gagal menambahkan jadwal: ${e.toString()}')));
    } finally {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Tambah Jadwal Baru')),
      body: SafeArea(
        child: Padding(
          ...
          child: Form(
            ...
            child: ListView(
              children: [
                ...
                const SizedBox(height: 16),
                const Text('Jam Keberangkatan', style: AppTextStyles.h3),
                const SizedBox(height: 8),
                TextFormField(
